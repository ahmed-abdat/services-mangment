...
export default function page({
  ...
  const name =  searchParams?.serviceName;
  
  const [serviceName, setServiceName] = useState<string>(name as string);
    ...
    try {
      ...
      if(name) {
        if(localServiceData.thumbnail.url === thumbnail.url) {
          await updateService(localServiceData.name, { name: serviceName });
        }else if (localServiceData.name === serviceName) {
          await updateService(localServiceData.name, {  thumbnail , oldThumbnail : localServiceData.thumbnail });
        }else {
          await updateService(localServiceData.name, { name: serviceName , thumbnail , oldThumbnail : localServiceData.thumbnail });
        }
        setTimeout(() => {
          setLoading(false);
